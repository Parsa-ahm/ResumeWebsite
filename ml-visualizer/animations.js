...
function animateDecisionTreePath(point) {
    ...
    d3.selectAll('.link').attr('stroke', 'rgba(255, 255, 255, 0.3)').attr('stroke-width', 2);
    
    // Highlight the data point being processed
    d3.selectAll('.data-point').each(function() {
        const el = d3.select(this);
        const title = el.select('title');
        if (title && title.text().includes(`Age: ${point.age}`) && title.text().includes(`Income: ${point.income}`)) {
    ...
    path.forEach((step, index) => {
        setTimeout(() => {
            if (step.node && !step.node.isLeaf()) {
                logToConsole(`[Step ${index + 1}] Evaluating: ${step.condition}`, 'info');
                
                // Find and highlight the node
                d3.selectAll('.decision-tree-node').each(function(d) {
                    if (d.data && (d.data.name.includes(step.condition) || d.data.name === step.condition)) {
                        d3.select(this)
                ...
                d3.selectAll('.link').each(function(d) {
                    if (d.target && d.target.data) {
                        ...
                        if (targetName.includes(step.condition) || step.condition.includes(targetName)) {
                            d3.select(this)
                              .transition()
                              .duration(300)
                              .attr('stroke', '#4ade80')
            ...
            if (index === path.length - 1) {
                // Final result
                logToConsole(`✓ Reached leaf node. Prediction: ${prediction}`, 'success');
...
function animateLinearRegressionPoint(point) {
    ...
    logToConsole(`[Step 1] Clicked point: (${point.x.toFixed(2)}, ${point.y.toFixed(2)})`, 'info');
    setTimeout(() => {
        logToConsole(`[Step 2] Calculating prediction using regression line...`, 'info');
        setTimeout(() => {
            logToConsole(`[Step 3] Predicted Y value: ${prediction.toFixed(2)}`, 'success');
            logToConsole(`[Step 4] Error: ${Math.abs(prediction - point.y).toFixed(2)}`, 'info');
            ...
            setTimeout(() => {
                ...
                if (meta.data[pointIndex]) {
                    ...
                    setTimeout(() => {
                        ...
                        logToConsole(`✓ Prediction complete! See the regression line above.`, 'success');
                    }, 500);
                }
            }, 500);
        }, 800);
...
function animateKNNClassification(point, k) {
    ...
    logToConsole(`[Step 1] Test point: (${point.x.toFixed(2)}, ${point.y.toFixed(2)})`, 'info');
    ...
    const distances = datasets.knn.map(d => ({
        point: d,
        distance: Math.sqrt(Math.pow(point.x - d.x, 2) + Math.pow(point.y - d.y, 2))
    }));
    ...
    setTimeout(() => {
        logToConsole(`[Step 2] Calculated distances to all ${datasets.knn.length} training points`, 'info');
        setTimeout(() => {
            ...
            kNearest.forEach((neighbor, index) => {
                setTimeout(() => {
                    logToConsole(`[Neighbor ${index + 1}] Distance: ${neighbor.distance.toFixed(3)}, Class: ${neighbor.point.class}`, 'info');
                    
                    // Find and highlight the point in the chart
                    const dataset = knnChart.data.datasets.find(d => d.label.includes(neighbor.point.class));
                    if (dataset) {
                        const pointIndex = dataset.data.findIndex(p => 
                            Math.abs(p.x - neighbor.point.x) < 0.01 && Math.abs(p.y - neighbor.point.y) < 0.01
                        );
                        if (pointIndex !== -1) {
                            // Animate highlight
                    ...
                    if (index === kNearest.length - 1) {
                        setTimeout(() => {
                            ...
                            const classCounts = {};
                            kNearest.forEach(n => {
                                classCounts[n.point.class] = (classCounts[n.point.class] || 0) + 1;
                            });
                            
                            logToConsole(`[Step 4] Votes: ${Object.entries(classCounts).map(([c, v]) => `${c}=${v}`).join(', ')}`, 'info');
                            logToConsole(`✓ Prediction: Class ${prediction} (majority vote)`, 'success');
...
function animateNeuralNetworkForward(inputs) {
    ...
    result.inputs.forEach((val, i) => {
        setTimeout(() => {
            ...
            neuron.transition().duration(500).attr('fill', val > 0.5 ? '#4a90e2' : '#1d2125');
        }, i * 200);
    });
    ...
    setTimeout(() => {
        ...
        outputNeuron.transition().duration(500).attr('fill', result.output > 0.5 ? '#4a90e2' : '#1d2125');
    }, hiddenDelay + 200);
}
...
function animateSVMBoundary() {
    ...
    const boundaryDataset = svmChart.data.datasets.find(d => d.label === 'Decision Boundary');
    if (boundaryDataset) {
